...
INPUT:
    ...
    resultPath: Path to folder where the result will be stored.
    
    imagesPath: Path to the folder where the images that are to be stitched 
                are stored. 
...
MIN_MATCH_COUNT = 10
nKeypoints = 5000  # number of keypoints the feature extractors will search for
ratioMatch = 0.75  # ratio used in the ratio test
algorithm = "SIFT" # String appended to the output image name to show the algorithm used

resultPath = "D:/Galeria/"
imagesPath = "D:/Galeria/imgs3/"

def parseArguments():
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('-rp', '--resultPath', help='Input folder path where the resulting panorama will be stored.')
    parser.add_argument('-ip', '--imagesPath', help='Input folder path where the input images are located.')
...
def fit_image_in_target_space(img_src, img_dst, mask, H, offset=np.array([0, 0, 0])):
    ...
    speed_get_pixel_val(img_dst, img_src, pts, out_src, offset)
...
def speed_get_pixel_val(img_dst, img_src, pts, out_src, offset):
    """
    :param img_dst:
    :param pts: pts for img_dst rows of [x1, y1]
    :param out_src: rows of [x1, y1], corresponding pts in src img after homography on dst points
    ...
    r_lzero = np.where(~np.any(tl < 0, axis=1))
    pts = pts[r_lzero[0], :]
    tl = tl[r_lzero[0], :]
    ...
    img_dst[pts[:,1], pts[:,0], :] = (img_src[tl[:,0], tl[:,1], :])


    return img_dst

def stitch(H_all, imgs, resultPath):
    ...
    for i, img in enumerate(imgs):
        ...
        result_path = join(resultPath, 'panorama' + algorithm + str(nKeypoints) + '.jpg')
...
def main():
    ...
    imgs = readImgsFromFolder(imagesPath)
    ...
    t1 = time.time_ns()-t0
    print("Time elapsed for keypoint and descriptor computation: " + str(t1/1000000000.))
    ...
    t2 = time.time_ns()-t1-t0
    print("Time elapsed for matches computation: " + str(t2/1000000000.))
    ...
    for i in range(len(matches)):
        ...
        cv.imwrite(join(resultPath,filename),imgMatches)
    """
    ...
    t3 = time.time_ns()-t2-t1-t0
    print("Time elapsed for homographies computation: " + str(t3/1000000000.))
    
    # Stitches the given images using the homographies computed
    
    stitch(H, imgs, resultPath)
    
    t4 = time.time_ns()-t3-t2-t1-t0
    print("Time elapsed for stitching process: " + str(t4/1000000000.))
